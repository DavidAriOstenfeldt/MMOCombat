...
@onready var ability = load("res://resources/abilities/pyroblast.tres")
@onready var hitbox: Area2D = $Hitbox
@onready var queue_free_timer: Timer = $QueueFreeTimer
@onready var pyroblast: GPUParticles2D = $Pyroblast
...
func on_hitbox_area_entered(other_area: Area2D) -> void:
	if other_area == target.hurtbox:
		...
		hit_ins.scale = Vector2(2,2)
		hit_ins.global_position = global_position
		get_parent().add_child(hit_ins)
		speed = 0.
		pyroblast.emitting = false
		pyroblast.visible = false
		sparks.emitting = false
		smoke.emitting = false
		queue_free_timer.wait_time = pyroblast.lifetime
